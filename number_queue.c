/**
* By Mat Sharff
* This queue is used to keep track of the available indexes in our arrays of process types
*/

#include "number_queue.h"

// const char * NUMBER_queue_string_header = "Q:Count=";
...
//
// const char * empty_NUMBER_queue_string = "Q:Count=0: -* : contents: Empty";

NUMBERq_p NUMBERq_construct() {
  NUMBERq_p queue = malloc(sizeof(NUMBERq));
...
int NUMBERq_init(NUMBERq_p queue, int max_initial_size) {
  // max initial size is the max amount of mutex variables we simulate
  int index;
  if (queue != NULL) {
    queue->size = 0;
    queue->head = NULL;
    queue->tail = NULL;
    for (index = 0; index < max_initial_size; index++) {
      NUMBERq_enqueue(queue, index);
    }
    return NO_ERRORS;
  }
  print_error(NULL_POINTER);
  printf("queue was null in NUMBERq_init");
  return NULL_POINTER;
}

int NUMBERq_enqueue(NUMBERq_p queue, int the_available_index) {
  if (queue != NULL) {
    number_node_p new_node = malloc(sizeof(number_node));
    new_node->available_index = the_available_index;
    new_node->next = NULL;

    if (NUMBERq_is_empty(queue)) {
      queue->head = new_node;
      queue->tail = new_node;
    } else {
  ...
  print_error(NULL_POINTER);
  printf("queue was null in NUMBERq_enqueue");
  return NULL_POINTER;
}

int NUMBERq_dequeue(NUMBERq_p queue) {
  number_node_p old_head;
  int result = -1;

  if (queue != NULL) {
    old_head = queue->head; //set old head to current head
    ...
    result = old_head->available_index;
    free(old_head); // Free the old head node
    return result;
  }
  print_error(NULL_POINTER);
  printf("queue was null in NUMBERq_dequeue");
  return result;
}

int NUMBERq_peek(NUMBERq_p queue) {
  if (queue != NULL) {
    return queue->head->available_index;
  }
  print_error(NULL_POINTER);
  printf("queue was null in NUMBERq_peek");
  return -1;
}

int NUMBERq_destruct(NUMBERq_p queue) {
  if (queue != NULL) {
    while (queue->size > 0) { //free the remaining nodes
      NUMBERq_dequeue(queue); // dequeue frees the node
    }
    free(queue); //free the queue
    return NO_ERRORS;
  }
  print_error(NULL_POINTER);
  printf("queue was null in NUMBERq_destruct");
  return NULL_POINTER;
}

int NUMBERq_is_empty(NUMBERq_p queue) {
  if (queue != NULL) {
    return (queue->size == 0);
  }
  print_error(NULL_POINTER);
  printf("queue was null in NUMBERq_is_empty");
  return -1;
}

int NUMBERq_size(NUMBERq_p queue) {
  if (queue != NULL) {
    return queue->size;
  }
  print_error(NULL_POINTER);
  printf("queue was null in NUMBERq_size");
  return -1;
}

// char * NUMBERq_to_string(NUMBERq_p queue) {
...
//   if (queue->size == 0) {
//     return_string = calloc(1, strlen(empty_NUMBER_queue_string) * sizeof(char) + 10);
//     strcat(return_string, empty_NUMBER_queue_string);
...
//     string_length = strlen(NUMBER_queue_string_header)
//         + digit_count
//         + strlen(colon_space)
...
//
//     //add "Q:Count=%d: "
//     sprintf(return_string, "%s%d%s", NUMBER_queue_string_header, queue_size, colon_space);
//
//     // This part adds all the P1->P2->P3 etc to the string
//     number_node_p current = queue->head;
//     while (current != NULL) {
...
// int NUMBERq_test(int test_size) {
//   int i;
//   NUMBERq_p queue = NUMBERq_construct();
//   NUMBERq_init(queue);
//   for (i = 0; i < test_size; i++) {
//     int temp = PCB_construct();
//     PCB_init(temp);
//     PCB_set_pid(temp, i);
//     printf("%s\n", PCB_to_string(temp));
//     NUMBERq_enqueue(queue, temp);
//     char * queue_string = NUMBERq_to_string(queue);
//     printf("%s\n", queue_string);
//     free(queue_string);
//   }
//   while (queue->size > 0) {
//     int temp = NUMBERq_dequeue(queue);
//     char * queue_string = NUMBERq_to_string(queue);
//     printf("%s\n", queue_string);
//     free(queue_string);
//     PCB_destruct(temp);
//   }
//   NUMBERq_destruct(queue);
// }
