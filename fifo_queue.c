...
size_t FIFOq_toString_size(FIFOq_p queue) {
    size_t header_size = 6;
    size_t PCBs_size = (size_t) (100 + 4) * FIFOq_size(queue);
    return header_size + PCBs_size + 1;
}

char* FIFOq_toString(FIFOq_p queue) {
    char* string = malloc(FIFOq_toString_size(queue));
    string[0] = '\0';   // I swear, this fixed so many freaking bugs -_-
    strcat(string, "Head: ");
    Node_p head = queue->front;
    while (head != NULL) {
        char* pcb_string = PCB_toString(head->pcb);
